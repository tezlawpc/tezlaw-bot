...
async function initTables() {
  ...
  // Anchor dates the jurisdiction rules compute from (post-trial motions,
  // appeal clocks, pre-cutoff discovery motions). The edit form has always
  // offered them, but the columns never existed, so they were dropped.
  for (const col of ["judgment_date", "judgment_notice_date", "verdict_date", "discovery_cutoff_date"]) {
    await db.query(`ALTER TABLE civil_cases ADD COLUMN IF NOT EXISTS ${col} DATE`).catch(() => {});
  }
...
async function updateCase(id, data) {
  const allowed = new Set([
    ...
    // The edit form sends these; before this they were silently discarded,
    // so changing a matter's jurisdiction or entering a judgment date did
    // nothing and the deadline chain never saw it.
    "jurisdiction","service_method","fee_arrangement_notes",
    "judgment_date","judgment_notice_date","verdict_date","discovery_cutoff_date",
  ]);
...
// ── Deleting a deadline ─────────────────────────────────────
//
// A MANUAL deadline is simply deleted. An AUTO-GENERATED one cannot be:
// the next "Regenerate" (or any change to a trigger date) upserts the whole
// chain by (case_id, source_trigger) and the deleted deadline would quietly
// come back. So an auto deadline is marked 'dismissed' instead — the upsert
// never touches status, so it stays gone — and it can be restored.
//
// Either way the case history records what was removed, by whom, and its
// due date. A deadline disappearing from a litigation file with no trace
// is exactly the kind of thing a malpractice carrier asks about.
async function deleteDeadline(id, user) {
  const r = await db.query(`SELECT * FROM civil_case_deadlines WHERE id = $1`, [id]);
  const d = r.rows[0];
  ...
  if (d.auto_generated) {
    await db.query(
      `UPDATE civil_case_deadlines SET status = 'dismissed', completed_at = NOW(), completed_by = $2 WHERE id = $1`,
      [id, user || null]
    );
    mode = "dismissed";
  } else {
    await db.query(`DELETE FROM civil_case_deadlines WHERE id = $1`, [id]);
    mode = "deleted";
  }

  try {
    await logEvent(d.case_id, {
      event_kind: "note",
      event_date: fmtDate(new Date()),
      title: `Deadline removed: ${d.description}`,
      description:
        `Due ${fmtDate(d.due_date)}${d.ccp_rule ? " · " + d.ccp_rule : ""} · was ${d.status}. ` +
        (mode === "dismissed"
          ? "Auto-generated, so it was dismissed rather than deleted — Regenerate will not bring it back. It can be restored from the removed list."
          : "Manual deadline, deleted."),
      created_by: user || null,
    });
  } catch (e) { /* the removal itself succeeded */ }

  return { ok: true, mode, deadline: d };
}

/** Put a completed or dismissed deadline back on the pending list. */
async function reopenDeadline(id, user) {
  const r = await db.query(
    `UPDATE civil_case_deadlines SET status = 'pending', completed_at = NULL, completed_by = NULL
      WHERE id = $1 RETURNING *`,
    [id]
  );
  const d = r.rows[0];
  ...
  try {
    await logEvent(d.case_id, {
      event_kind: "note",
      event_date: fmtDate(new Date()),
      title: `Deadline reopened: ${d.description}`,
      description: `Due ${fmtDate(d.due_date)}. Back on the pending list.`,
      created_by: user || null,
    });
  } catch (e) { /* non-fatal */ }
  return d;
...
async function listDeadlines(caseId, filter = {}) {
  ...
  const vals = [caseId];
  let i = 2;
  if (Array.isArray(filter.status)) { where.push(`status = ANY($${i++}::text[])`); vals.push(filter.status); }
  else if (filter.status) { where.push(`status = $${i++}`); vals.push(filter.status); }
  const r = await db.query(
    `SELECT * FROM civil_case_deadlines WHERE ${where.join(" AND ")}
     ORDER BY due_date ${filter.newestFirst ? "DESC" : "ASC"} LIMIT 200`,
    vals
  );
  return r.rows;
}
